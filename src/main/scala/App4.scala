...
/*
    App 4 is very similar to App 2 except that App 4 has an additional configuration. The driver memory is set to 10G, which
    is 1G by default (as in App 1 and App 2). App 4 is doing exactly the same operations as in App 1 and App 2. For further
    ...
    Total size of serialized results of 102 tasks (1026.5 MB) is bigger than spark.driver.maxResultSize (1024.0 MB)" due to
    "collect"
